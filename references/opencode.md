...
알 수 없는 설정 필드는 보존하지만 fallback 등은 Worker 실행에서 사용하지 않는다.
...
python3 scripts/worker.py set-default <provider/model> --variant <supported-variant>
python3 scripts/worker.py --project /absolute/repo set-project <provider/model> --variant <supported-variant>
...
```sh
python3 scripts/worker.py set-default <provider/model> --clear-variant
python3 scripts/worker.py --project /absolute/repo set-project null
python3 scripts/worker.py set-default null
```
...
```sh
python3 scripts/worker.py set-mode off
python3 scripts/worker.py set-mode auto
python3 scripts/worker.py --project /absolute/repo set-mode manual --project-only
python3 scripts/worker.py --project /absolute/repo set-mode inherit --project-only
```
...
```sh
python3 scripts/worker.py --project /absolute/repo run --brief /absolute/task.txt
python3 scripts/worker.py --project /absolute/repo run --brief /absolute/task.txt --inactivity-timeout 300
python3 scripts/worker.py --project /absolute/repo run --brief /absolute/task.txt --hard-timeout 7200
```
