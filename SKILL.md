...
Run `python3 <this-skill>/scripts/worker.py --project /absolute/repo run --brief /absolute/brief.txt --evidence-dir /outside/repo/evidence`. For an explicit $opencode-worker request, add `--explicit`; this never overrides off mode or host authorization. Use the saved model route. Add `--use-fallbacks` only when alternative routes are authorized; saved fallback routes alone do not enable them. `--no-fallback` takes precedence.
...
In a restricted Codex host, request normal host permission escalation on the FIRST OpenCode model lookup or Worker launch (for example exec_command with sandbox_permissions="require_escalated"). OpenCode needs its user log/cache/config directories, checkout lock and provider network access; repository write access alone is insufficient. Use the normal permission interface with the authorized task/provider scope. If rejected or missing authorization, stop and report it; never bypass rejection or replay possible side effects. Do not add per-tool approval relays.
...
Give brief conversational updates only at stage changes: delegation requested, execution confirmed when observed, result received with Worker-reported validation, and final Head outcome. For an actual correction, name the concrete issue and correction 1/1. Do not infer execution from a launch request, poll/read logs solely for display, narrate individual calls or repeat unchanged status.
